...
# 풀이의도
# 연도, 월, 일을 인자로 주어졌을 때, 요일을 출력하는
# printDayOfTheWeek() 함수를 구현하였으며
# datetime의 weekday()함수는 7개 요일 중 순서를 리턴하므로
# 해당 함수에서 요일배열을 미리 생성하여 한국의 요일이 나오도록 리턴함
...
def printDayOfTheWeek(myYear, myMonth, myDay):
    ...
    return dayString[datetime(myYear, myMonth, myDay).weekday()]
